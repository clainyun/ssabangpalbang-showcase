...
@WebMvcTest(KakaoOAuthCallbackController.class)
@Import(AuthSecurityConfiguration.class)
class KakaoOAuthCallbackControllerTest {
    ...
    @Test
    void 카카오_성공_callback은_code와_state를_앱으로_전달한다()
            throws Exception {
    ...
    @Test
    void 카카오_실패_callback은_provider_error를_앱으로_전달한다()
            throws Exception {
        mockMvc.perform(get("/oauth/kakao/callback")
                        .param("state", "oauth-state")
